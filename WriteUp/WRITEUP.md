...
| PR2             | On the tested devices, the app updates without noticeable delay when the user changes the graph or uses the algorithm controls. | 7/7 stakeholders did not mention performance as an issue                                        | There was no noticeable delay on any screens in my testing | <span style="color: green;"><strong>Met</strong></span>            |
...
| FR8<br><span style="color: orange;"><strong>Partially Met</strong></span> | The simulation shows when a graph cannot be solved by ending safely and displaying that no path was found. | I thought this was only partially met as a few of my stakeholders got confused. This was likely because the users got complacent when reading the explanation text which was the only indicator of the algorithm being solved. However the main issue was when the maximum connection length was set to zero the graph had no edges so no path was found instantly. The program allows you to set connection length to zero and does not warn the user of this.                     |
| FR10<br><span style="color: green;"><strong>Met</strong></span>           | Graphs should not be rendered with lines crossing each other where possible.                               | This was met due to my algorithm that chose the selected nodes to keep. However something that may have given the graphs more variety would be if it didn't always choose the shortest path. This would have meant longer connections and closer nodes that would be disconnected which could have made more interesting results for the algorithms; this would mainly influence the algorithms using heuristics as it would make the absolute distance heuristic not always useful. |
...
| ID                                                                        | Description                                                                                                | Thoughts                                                                                                                                                                                                                                                                                                |
| ------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FR3<br><span style="color: green;"><strong>Met</strong></span>            | The program correctly runs BFS, DFS, Greedy Best First Search, Dijkstra, and A* on fixed test graphs.      | I tested this using the trace table and they all produced the expected results. The test graphs were very useful as they gave different results for each algorithm.                                                                                                                                     |
| FR8<br><span style="color: orange;"><strong>Partially Met</strong></span> | The simulation shows when a graph cannot be solved by ending safely and displaying that no path was found. | The only indication was in the explanation box which some users found tedious to read each time. This was clearly not enough for the users as throughout the program they got used to expecting visual indicators (e.g. color changes). I would like to give clearer indications for all events in future.<br> |
...
| PR2<br><span style="color: green;"><strong>Met</strong></span>            | On the tested devices, the app updates without noticeable delay when the user changes the graph or uses the algorithm controls. | The app's performance was overall good. I was testing on my iPad mini 6 with an A15 Bionic chip and a stakeholder used a 9th gen iPad with an A13 Bionic and we both didn't notice any issues. |
